$(document).ready(function(){
    ...
    $(document).on('click','.cl_link',function(){
        ...
        var cn_res=confirm("Are you sure you want to link Diary no. "+t_h_cno+'/'+t_h_cyt+' with . '+hd_link.substr(0,hd_link.length-4)+'/'+hd_link.substr(-4));
        if(cn_res==true)
        {
            ...
            $.ajax({
                ...
                cache: false,
                async: true,
                data: {CSRF_TOKEN: CSRF_TOKEN_VALUE,d_no: t_h_cno,d_yr:t_h_cyt,hd_link:hd_link,hd_rec_date:hd_rec_date,hd_caveat_rec_dt:hd_caveat_rec_dt,check_caveat: 'Y',flag:'D'},
                type: 'GET',
...
//    var dv_mn_case=$('#dv_mn_case').html();
        ...
        var cn_res=confirm("Are you sure you want to unlink diary No "+ hd_linked_no.substr(0,hd_linked_no.length-4)+'/'+hd_linked_no.substr(-4)+' with '+ hd_caveat_no.substr(0,hd_caveat_no.length-4)+'/'+hd_caveat_no.substr(-4));
        if(cn_res==true)
        {
            $.ajax({
                ...
                cache: false,
                async: true,
                data: {hd_caveat_no: hd_caveat_no,hd_linked_no:hd_linked_no,sp_cav_diary_lnl_dt:sp_cav_diary_lnl_dt,flag:'D'},
//            beforeSend: function () {
//                $('#div_result').html('<table widht="100%" align="center"><tr><td><img src="../images/load.gif"/></td></tr></table>');
//            },
                type: 'GET',
...
function getDetails()
{
    ...
    var CSRF_TOKEN_VALUE = $('[name="CSRF_TOKEN"]').val();
    $.ajax({
        url: '/Caveat/Similarity/get_report_by_diary',
        cache: false,
        async: true,
        data: {CSRF_TOKEN: CSRF_TOKEN_VALUE,caveat_number: caveat_number,caveat_year:caveat_year,flag:'D'},
        beforeSend: function () {
            $('#div_result').html('<table widht="100%" align="center"><tr><td><img src="../../images/load.gif"/></td></tr></table>');
        },
        type: 'POST',
