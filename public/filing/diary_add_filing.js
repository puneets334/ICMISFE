...
// alert("Registry is directed not to accept any application or petition on behalf of:- \n 1) Suraj India Trust or Mr. Rajiv Daiya as per Hon'ble Court Order dated. 08-02-2018 in MA no. 1158/2017(Suraj India Trust Vs UOI) \n 2) ASOK PANDE as per Hon'ble Court Order dated. 26-10-2018 in WP(C) No. 965/2018 (ASOK PANDE Vs UOI) \n 3) MANOHAR LAL SHARMA  as per Hon'ble Court Order dated. 07-12-2018 in WP(CRL) No. 315/2018 (MANOHAR LAL SHARMA Vs ARUN JAITLEY (AT PRESENT FINANCE MINISTER))");
    ...
    var sci_waring_alert="Registry is directed not to accept any application or petition on behalf of:- \n 1) Suraj India Trust or Mr. Rajiv Daiya as per Hon'ble Court Order dated. 08-02-2018 in MA no. 1158/2017(Suraj India Trust Vs UOI) \n 2) ASOK PANDE as per Hon'ble Court Order dated. 26-10-2018 in WP(C) No. 965/2018 (ASOK PANDE Vs UOI) \n 3) MANOHAR LAL SHARMA  as per Hon'ble Court Order dated. 07-12-2018 in WP(CRL) No. 315/2018 (MANOHAR LAL SHARMA Vs ARUN JAITLEY (AT PRESENT FINANCE MINISTER))\n" +
...
//priority added on 21-08-2023
    ...
    //alert($("select[name='ddl_priority'] option:selected").index());
...
//alert(ext_address);
    var url = "/Filing/Diary/save_new_filing?controller=I"  + "&st_status=" + st_status+
        "&ddl_st_agncy="+ddl_st_agncy+"&ddl_bench="+ddl_bench+"&hd_mn="+hd_mn+"&cs_tp="+cs_tp+"&txtFNo="+txtFNo+"&txtYear="+txtYear+
...
function get_sc_District(side, id, val,hd_city) {
    ...
    xmlhttp.open("GET", "/Common/Ajaxcalls/get_districts?state_id=" + val+"&city="+hd_city, true);
    xmlhttp.send(null);
}
...
//    }
    ...
    var url = "/Filing/Diary/save_new_filing?controller=I" + '&txt_p_no=' + txt_p_no + '&txt_yr=' + txt_yr + "&st_status=" + st_status;
   ...
   /* // This below code not used confirmed by Mrs.Vandana and Mr.Pavan sir on 15 Feb 2024
    $(document).on('change', '#ddl_bench', function() {
        ...
        if(ddl_court.length == 0)
        {
            ...
            return false;
        }else if(ddl_st_agncy.length == 0)
        {
            alert("Please select court state");
            $('#ddl_st_agncy').focus();
            validationError = false;
            return false;
        }else if(ddl_bench.length == 0)
        {
            alert("Please select court bench");
            $('#ddl_bench').focus();
            validationError = false;
            return false;
        }
        ...
        $.ajax({
            url: 'Filing/Diary/get_case_strc',
            cache: false,
            async: true,
            data: {ddl_st_agncy: ddl_st_agncy, ddl_bench: ddl_bench,ddl_court:ddl_court,CSRF_TOKEN:CSRF_TOKEN_VALUE},
            beforeSend: function() {
...
//          alert(idd);
        ...
        //alert('anshu additional_address');
        $.ajax({
...
function get_sc_Details()
{
    ...
    $.ajax({
        url: '/Filing/Diary/get_sc_parties',
        cache: false,
        async: true,
...
//alert(" this is save button");
    ...
    if((dno !=''))
    {
        ...
        data1=dno+dyr;
        url = "/Filing/Diary/getdiary_no?q="+data1;

    }
    else

    {
        url = "/Filing/Diary/getdiary_no?q=" + data;
    }
    //get_sc_Details();
    if (window.XMLHttpRequest)
    {
