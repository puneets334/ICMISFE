...
use App\Models\Copying\CopyingModel;
use App\Models\Common\DropdownListModel;
use App\Models\Entities\ModelPostBarCodeMapping;
use App\Models\Entities\CopyingModelOrderIssuingApplicationNew;
use App\Models\Entities\CopyingModelRequestVerify;
use App\Models\Entities\CopyingModelApplicationDocuments;
use App\Models\Entities\CopyingModelApplicationDocumentsA;
use App\Models\Entities\CopyingModelRequestVerifyDocuments;
use App\Models\Entities\CopyingModelReasonsForRejection;
use App\Models\Entities\CopyingModelRole;
use App\Models\Entities\Users;
use App\Models\Entities\CopyingModelTrap;
use App\Models\Entities\CopyingModelApplicationDefects;
...
class Copying extends BaseController
{
    ...
    public $CopyingModelOrderIssuingApplicationNew;
    public $CopyingModelRequestVerify;
    public $CopyingModelRequestVerifyDocuments;
    public $CopyingModelReasonsForRejection;
    public $CopyingModelRole;
    public $CopyingModelApplicationDocuments;
    public $CopyingModelApplicationDocumentsA;
    public $Model_users;
    public $CopyingModelTrap;
    public $CopyingModelApplicationDefects;
    ...
    function __construct()
    {
        ...
        $this->Copying_model = new CopyingModel();
        $this->PostBarCodeMapping = new ModelPostBarCodeMapping();
        $this->CopyingModelOrderIssuingApplicationNew = new CopyingModelOrderIssuingApplicationNew();
        $this->CopyingModelRequestVerify = new CopyingModelRequestVerify();
        $this->CopyingModelRequestVerifyDocuments = new CopyingModelRequestVerifyDocuments();
        $this->CopyingModelReasonsForRejection = new CopyingModelReasonsForRejection();
        $this->CopyingModelRole = new CopyingModelRole();
        $this->CopyingModelApplicationDocuments = new CopyingModelApplicationDocuments();
        $this->CopyingModelApplicationDocumentsA = new CopyingModelApplicationDocumentsA();
        $this->Model_users = new Users();
        $this->CopyingModelTrap = new CopyingModelTrap();
        $this->CopyingModelApplicationDefects = new CopyingModelApplicationDefects();
    }
    ...
    public function download_order()
    {
        ...
        if (!empty($file_path_row)) {
            ...
            $data = file_get_contents('http://10.25.78.60/supreme_court/jud_ord_html_pdf/' . $file_path);
    ...
    public function copy_search()
    {
        ...
        $data['copy_category'] = $this->DropdownListModel->get_copy_category();
    ...
    public function get_copy_search()
    {
        ...
        //$this->CopyingModelOrderIssuingApplicationNew = new CopyingModelOrderIssuingApplicationNew();
        $this->CopyingModelOrderIssuingApplicationNew->join('main as m', 'm.diary_no = copying_order_issuing_application_new.diary', 'left');
        $this->CopyingModelOrderIssuingApplicationNew->join('main_a as ma', 'ma.diary_no = copying_order_issuing_application_new.diary', 'left');
        $this->CopyingModelOrderIssuingApplicationNew->join('master.ref_copying_source as r ', 'r.id = copying_order_issuing_application_new.source', 'left');
        $this->CopyingModelOrderIssuingApplicationNew->join('master.ref_copying_status as s ', 's.status_code = copying_order_issuing_application_new.application_status', 'left');
        $this->CopyingModelOrderIssuingApplicationNew->select('(case when m.reg_no_display is not null then m.reg_no_display else 
        ...
        if ($flag == 'ano') {
            $this->CopyingModelOrderIssuingApplicationNew->where(['application_reg_number' => $application_no, 'application_reg_year' => $application_year, 'copy_category' => $application_type]);
            $data['result'] = $this->CopyingModelOrderIssuingApplicationNew->findAll();
            $query = $this->db->getLastQuery();

            $data['application_request'] = 'application';
        } else {
            $this->CopyingModelOrderIssuingApplicationNew->where(['crn' => $crn]);
            $data['result'] = $this->CopyingModelOrderIssuingApplicationNew->findAll();
//
            $data['application_request'] = 'application';

            if (empty($data['result'])) {
                $this->CopyingModelRequestVerify->join('main m', 'm.diary_no = copying_request_verify.diary', 'left');
                $this->CopyingModelRequestVerify->join('main_a ma', 'ma.diary_no = copying_request_verify.diary', 'left');
                $this->CopyingModelRequestVerify->join('master.ref_copying_source as r ', 'r.id = copying_request_verify.source', 'left');
                $this->CopyingModelRequestVerify->join('master.ref_copying_status as s ', 's.status_code = copying_request_verify.application_status', 'left');
                $this->CopyingModelRequestVerify->select('(case when m.reg_no_display is not null then m.reg_no_display else 
                                    ma.reg_no_display end) as reg_no, (case when m.c_status is not null then m.c_status else 
                ...
                $data['result'] = $this->CopyingModelRequestVerify->where(['crn' => $crn])->findAll(1);
        ...
        if (is_array($data['result'])) {
            foreach ($data['result'] as $row) {
                ...
                if ($data['application_request'] == 'request') {
                    ...
                    $this->CopyingModelRequestVerifyDocuments->join('master.ref_order_type r', 'copying_request_verify_documents.order_type = r.id', 'left');
                    $data['row1'] = $this->CopyingModelRequestVerifyDocuments->where(['copying_order_issuing_application_id' => $row['id']])->findAll();
                }
            }
        }
    ...
    public function reason_rejection()
    {
        ...
        $this->CopyingModelReasonsForRejection->join('master.users u', 'copying_reasons_for_rejection.user_id = u.usercode', 'left');
        $this->CopyingModelReasonsForRejection->select('copying_reasons_for_rejection.*,u.name as user_name');

        $data['rejection_reasons'] = $this->CopyingModelReasonsForRejection->where(['is_active' => 'T'])->findAll();
    ...
    public function reason_reject_save()
    {
        ...
        $supdated = $this->CopyingModelReasonsForRejection->update($md5_reason, $dataArray);
        if ($supdated) {
    ...
    public function reason_reject_insert()
    {
        ...
        if (intval($user_id) > 0) {
            if ($reject_reasons != '') {
                ...
                $this->CopyingModelReasonsForRejection->select('id');

                $data['reason_already_existed'] = $this->CopyingModelReasonsForRejection->where(['reasons' => $reject_reasons])->findAll(1);
               //  $query=$this->db->getLastQuery();
               //  echo (string) $query;exit();
                ...
                if (empty($data['reason_already_existed'])) {
                    ...
                    $this->db = \Config\Database::connect();
                    $this->db->transStart();
                    $this->CopyingModelReasonsForRejection->insert($dataArray);
                    $this->db->transComplete();
                    $response = '1';
    ...
    {
        $this->CopyingModelRole->join('master.users u', 'copying_role.role_assign_by = u.usercode', 'left');
        $this->CopyingModelRole->select('copying_role.*,u.name as role_assined_by');

        $data['user_roles'] = $this->CopyingModelRole->where(['status' => 'T'])->findAll();
    ...
    public function user_role_add()
    {
        ...
        if ($user_id == 1) {
            ...
            $data['users_data'] = $this->DropdownListModel->get_copying_users($user_type_section);
        }
        //print_r($users_data);
    ...
    public function role_assign_add()
    {
        ...
        if (isset($user_id) != '') {
            if ($applicantType != '' && $role_assign != '') {
                $this->CopyingModelRole->select('id');
                $data['role_already_assigned'] = $this->CopyingModelRole->where(['role_assign_to' => $role_assign])->where(['status' => 'T'])->findAll();
                if (empty($data['role_already_assigned'])) {
                    ...
                    $this->db = \Config\Database::connect();
                    $this->db->transStart();
                    $this->CopyingModelRole->insert($dataArray);
                    $this->db->transComplete();
                    $response = '1';
    ...
    public function get_application_search()
    {
        ...
        $this->CopyingModelOrderIssuingApplicationNew->where(['source' => 6]);
        if (!empty($application_type)) {
            $this->CopyingModelOrderIssuingApplicationNew->whereIn('copy_category', $application_type);
        }

        if (!empty($applicant_type)) {
            $this->CopyingModelOrderIssuingApplicationNew->whereIn('filed_by', $applicant_type);
        }
        $this->CopyingModelOrderIssuingApplicationNew->whereNotIn('application_status', ['F', 'R', 'D', 'C', 'W']);

        $this->CopyingModelOrderIssuingApplicationNew->where("DATE(copying_order_issuing_application_new.application_receipt) BETWEEN '$from_date' and '$to_date'");
        $data['all_application_search'] = $this->CopyingModelOrderIssuingApplicationNew->findAll();
        $user_verification_details = [];
        if (is_array($data['all_application_search'])) {
            foreach ($data['all_application_search'] as $row) {

                $this->CopyingModelApplicationDocuments->join('master.ref_order_type r', 'copying_application_documents.order_type = r.id', 'left');
                $this->CopyingModelApplicationDocuments->select('r.order_type order_name, copying_application_documents.*');
                $this->CopyingModelApplicationDocuments->where(['copying_order_issuing_application_id' => $row['id'], 'sent_to_applicant_on' => null]);

                $all_documents = $this->CopyingModelApplicationDocuments->findAll();
    ...
    public function bulk_status_get_data()
    {
        ...
        if (!empty($from_date) && !empty($to_date)) {
            ...
            if ($usercode != null && $from_date != '1970-01-01' && $to_date != '1970-01-01') {
                $this->CopyingModelOrderIssuingApplicationNew->join('master.users u', 'u.usercode=copying_order_issuing_application_new.adm_updated_by', 'left');
                $this->CopyingModelOrderIssuingApplicationNew->select("copy_category,application_reg_number,copying_order_issuing_application_new.id,application_number_display,diary,
                ...
                $this->CopyingModelOrderIssuingApplicationNew->where("DATE(copying_order_issuing_application_new.updated_on) BETWEEN '$from_date' and '$to_date'");
                $this->CopyingModelOrderIssuingApplicationNew->where(['application_status' => 'P', 'source' => 1]);
                if (intval($usercode) > 0) {
                    $this->CopyingModelOrderIssuingApplicationNew->where(['usercode' => $usercode]);
                ...
                $this->CopyingModelOrderIssuingApplicationNew->orderBy('date(application_receipt),copy_category,application_reg_number');
    ...
    function application_status()
    {
        ...
        if (isset($app_no) &&  $this->request->getPost('app_no') != '') {
            ...
            $this->CopyingModelOrderIssuingApplicationNew->join('main as m', 'm.diary_no = copying_order_issuing_application_new.diary', 'left');
            $this->CopyingModelOrderIssuingApplicationNew->join('main_a as ma', 'ma.diary_no = copying_order_issuing_application_new.diary', 'left');
            $this->CopyingModelOrderIssuingApplicationNew->join('master.users as u ', 'u.usercode=m.dacode', 'left');
            $this->CopyingModelOrderIssuingApplicationNew->join('master.usersection as us ', 'us.id=u.section', 'left');
            // $this->CopyingModelOrderIssuingApplicationNew->join('copying_application_documents as b', 'copying_order_issuing_application_new.id = b.copying_order_issuing_application_id', 'left');
            // $this->CopyingModelOrderIssuingApplicationNew->join('copying_application_documents_a as ba ', 'copying_order_issuing_application_new.id = ba.copying_order_issuing_application_id', 'left');
            $this->CopyingModelOrderIssuingApplicationNew->select("copying_order_issuing_application_new.*,
 ...
 tentative_section(case when m.diary_no is null then ma.diary_no else m.diary_no end)  end) as section_name");
            $this->CopyingModelOrderIssuingApplicationNew->where(['application_number_display' => $app_no]);
            $data['application_details'] = $this->CopyingModelOrderIssuingApplicationNew->findAll();
            ...
            if (!empty($data['application_details'])) {
                ...
                $this->CopyingModelTrap->join('master.users as u ', 'u.usercode=copying_trap.updated_by', 'left');
                $this->CopyingModelTrap->join('master.ref_copying_status as prev ', 'prev.status_code = copying_trap.previous_value', 'left');

                $this->CopyingModelTrap->join('master.ref_copying_status as new ', 'new.status_code=copying_trap.new_value', 'left');
                $this->CopyingModelTrap->select('prev.status_description as prev,new.status_description as new,
                name,empid,copying_trap.updated_on');
                $this->CopyingModelTrap->where(['copying_application_id' => $application_details_id]);

                $data['trap_list'] = $this->CopyingModelTrap->findAll();
                //  $query=$this->db->getLastQuery();
                //  echo (string) $query;

                $query = $this->CopyingModelApplicationDefects->select('ref_order_defect_id,remark')
    ...
    public function application_status_update()
    {
        ...
        if (!empty($app_id)) {
            ...
            if ($application_status == 'R') {
                ...
                $supdated = $this->CopyingModelOrderIssuingApplicationNew->update($app_id, $dataArray);
                ...
                $this->CopyingModelApplicationDefects->set($dataDefectsArray)
...
//             }
    ...
    public function dashboard_count()
    {
        ...
        $query = $this->CopyingModelOrderIssuingApplicationNew;
        $query->select([
        ...
        $query_pending = $this->CopyingModelOrderIssuingApplicationNew;
        ...
        $query3 = $this->CopyingModelRequestVerify;
        $query3->select([
    ...
    public function dashboard_details(){
        ...
        if($flag == 'offline_total_applications' OR $flag == 'offline_pending_applications' OR $flag == 'offline_disposed_applications') {
            $table = $this->CopyingModelOrderIssuingApplicationNew;
            ...
            $table = $this->CopyingModelRequestVerify;
        ...
        if(isset($diary_number, $diary_year, $category, $deliver_mode, $advocate_or_party, $court_fee, $order_type)){
            ...
            if($isInserted){
                ...
                for($i=0; $i < sizeof($order_type); $i++){
                    ...
                    $this->CopyingModelApplicationDocuments->insert($tempArray);
                }
