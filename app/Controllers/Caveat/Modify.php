...
class Modify extends BaseController
{
    ...
    {   $caveat_no=$_SESSION['caveat_details']['caveat_no'];
        ...
        $data['state'] = $this->DropdownListModel->get_address_state_list();
        ...
        $data['casetype']=$this->DropdownListModel->get_case_type('filing','nature_sci');
        $data['casetype_nature_sci']=$this->DropdownListModel->get_case_type('filing','nature_sci');
        $data['role']=$role;
        $data['is_c_status']='P';
        ...
        if (!empty($data['is_c_status']) && $data['is_c_status']=='P'){
            $data['update_button']="<input type='button' class='btn btn-success' value='Save' onclick='call_update_main()' id='svbtn' onkeydown='if (event.keyCode == 13) document.getElementById('svbtn').click()'>";
        }
    ...
    public function save_caveat(){
        ...
        if($_REQUEST['controller']=='U') {
            ...
            <table align="center" width="100%">
                <tr><th>Record Updated Successfully</th></tr>
                <th>Caveat No.:<h2 style="color: blue"><?php echo substr($caveat_details['caveat_no'], 0, -4).'/'.substr($caveat_details['caveat_no'],-4);?></h2></th></tr>
