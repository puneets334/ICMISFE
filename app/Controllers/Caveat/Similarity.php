...
class Similarity extends BaseController
{
  ...
  (CURRENT_DATE -diary_no_rec_date::date) AS no_of_days,casetype_id,caveat_no,ref_agency_state_id")->where(['caveat_no'=>$caveat_no])->get()->getRowArray();
            ...
            if (!empty($mainCaveat)){
                ...
                $data['mainCaveat']=$mainCaveat;
                $data['caveatSBCJ']= $this->ModelSimilarity->get_SBCJ($caveat_no,$is_order_challenged);
                $data['caveatSBC']= $this->ModelSimilarity->get_SBC($caveat_no,$is_order_challenged);
                $data['caveatSBJ']= $this->ModelSimilarity->get_SBJ($caveat_no,$is_order_challenged);
                if($casetype_id=='7'|| $casetype_id=='8' || $casetype_id=='5' || $casetype_id=='6')
                {
                    $data['caveatSCC']= $this->ModelSimilarity->get_SCC($caveat_no);
                }
                if($casetype_id=='24') {
                    $data['arbitration'] = $this->ModelSimilarity->get_arbitration($caveat_no,$is_order_challenged);
                    $data['arbitration_ref_date'] = $this->ModelSimilarity->get_arbitration_ref_date($caveat_no,$is_order_challenged);
                    $data['arbitration_date'] = $this->ModelSimilarity->get_arbitration_date($caveat_no,$is_order_challenged);
                }
            }
        }
                ...
                if (!empty($is_caveat_diary_matching)){
                    foreach ($is_caveat_diary_matching as $row){
                        if (!empty($row['caveat_no'])){
                            $is_check_caveat_same_party=$this->ModelSimilarity->check_caveat_same_party($row['caveat_no'],$diary_no);
                            if ($is_check_caveat_same_party){
                                $is_check_caveat_same_party=$this->ModelSimilarity->check_caveat_same_party($row['caveat_no'],$diary_no,'_a');
                            }
  ...
  (CURRENT_DATE -diary_no_rec_date::date) AS no_of_days,casetype_id,diary_no as caveat_no,ref_agency_state_id,diary_no")->where(['diary_no'=>$caveat_no])->get()->getRowArray();
            ...
            if (!empty($mainCaveat)){
                ...
                $data['mainCaveat']=$mainCaveat;
                $data['caveatSBCJ']= $this->ModelSimilarity->get_SBCJ($caveat_no,$is_order_challenged,$flag);
                $data['caveatSBC']= $this->ModelSimilarity->get_SBC($caveat_no,$is_order_challenged,$flag);
                $data['caveatSBJ']= $this->ModelSimilarity->get_SBJ($caveat_no,$is_order_challenged,$flag);
                if($casetype_id=='7'|| $casetype_id=='8' || $casetype_id=='5' || $casetype_id=='6')
                {
                    $data['caveatSCC']= $this->ModelSimilarity->get_SCC($caveat_no,$flag);
                }
                if($casetype_id=='24') {
                    $data['arbitration'] = $this->ModelSimilarity->get_arbitration($caveat_no,$is_order_challenged,$flag);
                    $data['arbitration_ref_date'] = $this->ModelSimilarity->get_arbitration_ref_date($caveat_no,$is_order_challenged,$flag);
                    $data['arbitration_date'] = $this->ModelSimilarity->get_arbitration_date($caveat_no,$is_order_challenged,$flag);
                }
            }
        }
        $data['flag']=$flag;
