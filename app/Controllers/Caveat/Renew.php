...
class Renew extends BaseController
{
  ...
  (CURRENT_DATE -diary_no_rec_date::date) AS no_of_days,casetype_id")->where(['caveat_no'=>$caveat_no])->get()->getResultArray();
            if ($cause_title){
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (!empty($from_caveat_no)){
                foreach ($from_caveat_no as $caveat_row) {
                    ...
                    unset($caveat_row['create_modify']);unset($caveat_row['updated_by']);unset($caveat_row['updated_by_ip']);
