...
use App\Controllers\Filing\FileTrap;
...
class Report extends BaseController
{
    ...
    {
        return view('Reports/filing/report');
    }
        ...
        if (!empty($type)) {
            ...
            switch ($type) {
                ...
                case 8: //Dynamic Search
                    ...
                    $data['caseTypes'] = $this->DropdownListModel->get_case_type_list();
                    $data['Sections'] = $this->DropdownListModel->getSections();
                    $data['MCategories'] = $this->DropdownListModel->getMainSubjectCategory();
                    $data['states'] = $this->DropdownListModel->get_state();
                    $data['judges'] = $this->DropdownListModel->get_judges();
                    $data['aors'] = $this->DropdownListModel->get_aor();
                ...
                case 18: //Change Category Report
                    ...
                    $data['sections'] = $this->DropdownListModel->get_work_done_ib_section($empid);
    ...
    public function fil_trap_search()
    {
        ...
        if ($data['incompleteandcompletematter'] == 'cv') {
            ...
            $FileTrap = new FileTrap();
            $result = $FileTrap->index();
            echo $result;
            exit();
        } else {
    ...
    {
        $data_array = $this->DropdownListModel->get_casetype($this->request->getGet('type'));
    ...
    public function get_Sub_Subject_Category()
    {
        ...
        $data_array = $this->DropdownListModel->get_Sub_Subject_Category($this->request->getGet('Mcat'));
        echo json_encode($data_array);
    }

    public function get_da()
    {
        $data_array = $this->DropdownListModel->get_da($this->request->getGet('section'));
        echo json_encode($data_array);
    }

    public function get_agency()
    {
        $data_array = $this->DropdownListModel->get_agency_code($this->request->getGet('state'), $this->request->getGet('agency'));
        echo json_encode($data_array);
    }
    ...
    public function uoi_slp_search()
    {
        ...
        $table = '<table border="1px">
                    <tbody><tr><th colspan="9">UOI as Petitioner/Respondent SLPs</th></tr>
                    <tr><th>Filing Year</th><th>UOI SLPs</th> <th>Total Matters</th></tr>';
        foreach ($uoi_slp_counts as $row) {
            $table .= '<tr><td>' . $row['filing_year'] . '</td><td>' . $row['matched'] . '</td><td>' . $row['total'] . '</td></tr>';
        }
        $table .= '</tbody></table>';
        echo $table;
    }
...
<table align="center"  border="1" style="border-collapse: collapse"><tr>
        <th>SNo.</th>
         <th>Name</th><th>' . (($report_for == 1) ? 'Total Diary No Category Changed' : 'Total Auto Linked Cases Changed') . '</th></tr>';
            $sno = 0;
            foreach ($report_data as $row) {
                $table .= '<tr><td>' . ++$sno . '</td><td>' . $row['name'] . '</td>
<td><input type="hidden" name="hd_usr_id' . $sno . '" id="hd_usr_id' . $sno . '" value="' . $row['usercode'] . '"/><span id="sp_tot_cases' . $sno . '" class="cl_add_cst">' . $row['s'] . '</span></td>
    ...
    public function get_complete_filing_report(){
        ...
        if (sizeof($result_array)) {
            $table = '<div style="margin-top: 30px"></div>
                    <table style="margin-left: auto;margin-right: auto;border-collapse:collapse;margin-top: 10px" border="1" cellspacing="9" cellpadding="7">
                        <tr><th>Total Filed</th><th>Counter Filed</th><th>e- Filed</th><th>Re-filed</th><th>Registered</th><th>Checked Verification</th><th>Verified</th></tr>
                        <tr><th>'.$result_array['filed'].'</th><th>'.$result_array['counter_filed'].'</th><th>'.$result_array['e_filed'].'</th><th>'.$result_array['refiled'].'</th><th>'.$result_array['registered'].'</th><th>'.$result_array['t_crawl_veri'].'</th><th>'.$result_array['verified'].'</th></tr>
                    </table><br><br>
                    <table style="margin-left: auto;margin-right: auto;border-collapse:collapse;margin-top: 20px" border="1" cellspacing="9" cellpadding="7">
                        <tr><th colspan="2">Total Pendency till above Date</th></tr>
                        <tr><th>Pending for Tagging Verification</th><th>Pending Verification after Re-filing</th></tr>
                        <tr><th>'.$result_array['pending_tagging'].'</th><th><span onclick="get_complete_filing_details('.$report_for.',1)">'.$result_array['pending_ver_aft_ref'].'</span> ( <span onclick="get_complete_filing_details('.$report_for.',2)">'.$result_array['pending_ver_aft_ref_registered'].' Reg)</span></th></tr>
                    </table>';
        }
        echo $table;
    }
    ...
    function get_rcc_count_report(){
        ...
        $table .= '<table id="diaryReport" width="100%" border="1" cellspacing="1">
                    <thead>
                    <tr bgcolor="#dcdcdc">
        ...
        if(sizeof($result_array)>0){
            ...
            $table .= '<tr><td colspan="2" style="text-align: center;font-weight:bold">Total:</td><td style="text-align: center;font-weight:bold">'.$total.'</td></tr>';
        }
        echo $table;
    }
    ...
    public function get_rcc_case_details(){
        ...
        if(sizeof($report_data)){
            $table = '<div style="margin-top: 30px"></div><table id="diaryReport" width="100%" border="1" cellspacing="1">
                        <thead>
                        <tr bgcolor="#dcdcdc">
                            <th style="text-align: center;">Sr.No.</th>
