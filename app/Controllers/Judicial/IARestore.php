...
class IARestore extends BaseController
{
    public function jk(){
        // exit('hello');
    ...
    public $IARestoreModel;

    function __construct(){
        $this->DropdownListModel= new DropdownListModel();
        $this->IARestoreModel = new IARestoreModel();
    }
    ...
    public function get_content_ia_alive_process(){
        ...
        if (!empty($diary_no)){
            ...
            $is_docdetails = $this->IARestoreModel->get_docdetails($diary_no);
            if (empty($is_docdetails)){
                $is_docdetails = $this->IARestoreModel->get_docdetails($diary_no,'','_a');
            }
        }
    ...
    public function ia_alive_process(){
        ...
        if ($option == 2) {
            ...
            for ($i = 0; $i < $len; $i++) {
                ...
                $docdetails = $this->IARestoreModel->get_docdetails($diary_no,[$doc_id[$i]]);
                if (empty($docdetails)){
                    $docdetails = $this->IARestoreModel->get_docdetails($diary_no,[$doc_id[$i]],'_a');
                }
                if (!empty($docdetails)){
            ...
            $docdetails2 = $this->IARestoreModel->get_docdetails($diary_no,$doc_id);
            if (empty($docdetails2)){
                $docdetails2 = $this->IARestoreModel->get_docdetails($diary_no,$doc_id,'_a');
