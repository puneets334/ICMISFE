...
class Sentence extends BaseController
{
    ...
    public function get_sentence_undergone_list(){
        ...
        $data['sentence_undergone_list']= $this->ModelSentence->get_sentence_undergone_list($sentence_period_id);
    ...
    public function delete_sentence_undergone(){
        ...
        if (!empty($sentence_undergone_id)){
            ...
            $is_update_sentence_undergone= $this->ModelSentence->update_sentence_undergone($updateData,$sentence_undergone_id);
    ...
    public function add_details()
    {
        if ($this->request->getMethod() === 'post') {
            ...
            $is_sentence_period= $this->ModelSentence->is_sentence_period($data);
    ...
    public function add_period_undergone()
    {
        if ($this->request->getMethod() === 'post') {
            ...
            $this->db->transStart();
            $get_sentence_period= $this->ModelSentence->get_sentence_period($data);
            ...
            if ($effected_sentence_period){
                $check_sentence_period= $this->ModelSentence->get_sentence_period($data);
    ...
    public function get_details(){
        ...
        $data['lct_dec_dt']= $this->ModelSentence->get_lct_dec_dt($params);
        $data['get_details']= $this->ModelSentence->get_details($params);
        if (!empty($data['get_details'])){
            $sentence_period_id=$data['get_details']['id'];
            $get_res_max_to_dt= $this->ModelSentence->get_max_to_dt($sentence_period_id);
    ...
    public function get_from_court_by_diary_no()
    {
        ...
        $from_cout=$this->ModelSentence->get_from_court_by_diary_no($diary_no);
    ...
    public function get_state_name()
    {
        ...
        $from_data=$this->ModelSentence->get_state_name($diary_no,$ddl_court);
        //echo '<pre>';print_r($from_cout);exit();
        $selected = "";
        $dropDownOptions = '<option value="">Select Court Type</option>';
    ...
    public function get_index_bench()
    {
        ...
        $from_data=$this->ModelSentence->get_sentence_bench($params);
        $selected = "";
    ...
    public function get_tot_cases()
    {
        ...
        $from_data=$this->ModelSentence->get_tot_cases($params);
        $selected = "";
    ...
    public function get_tot_accused()
    {
        ...
        $from_data=$this->ModelSentence->get_tot_accused($params);
        $selected = "";
        $dropDownOptions = '<option value="">Select</option>';
        if (!empty($from_data) && !empty($from_data)) {
            foreach ($from_data as $row) {
