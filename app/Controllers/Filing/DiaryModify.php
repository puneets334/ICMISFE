...
class DiaryModify extends BaseController
{
    public $LoginModel;
    ...
    public function index()
    {
        ...
        $data['casetype'] = $this->DropdownListModel->get_case_type('filing', 'nature_sci'); // doubts discussion
        ...
        $data['hc_benches'] = $this->DropdownListModel->get_ref_agency_code($fetch_rw['ref_agency_state_id'], $fetch_rw['from_court']);
        ...
        if (!empty($data['pet_rw']) && !empty($data['pet_rw']['city'])) {
            $data['pet_dist_list'] = $this->DropdownListModel->get_districts_list($data['pet_rw']['city']);
        }
        $data['res_rw'] = $this->Model_diary->get_party_list($diary_no, 'R');
        if (!empty($data['res_rw']) && !empty($data['res_rw']['city'])) {
            $data['res_dist_list'] = $this->DropdownListModel->get_districts_list($data['res_rw']['city']);
        }
        ...
        if (!empty($fetch_rw)) {
            if (!empty($fetch_rw['c_status']) && $fetch_rw['c_status'] == 'P') {
                $data['update_button'] = "<input type='button' class='btn btn-success' value='Save' onclick='call_update_main()' id='svbtn' onkeydown='if (event.keyCode == 13) document.getElementById('svbtn').click()' ><input type='button' class='btn btn-danger' value='Cancel' onclick='window.location.reload()'>";
            }
        }
...
?>
            <div style="padding: 30px;">
                <table align="center" width="100%" style="text-align: center;">
                    <tr>
                        <th style="color: green;">Record Updated Successfully</th>
                    </tr>
                    <th>Diary No.:<h2 style="color: blue"><?php echo $_REQUEST['d_no'] . '/' . $_REQUEST['d_yr']; ?></h2>
                    </th>
                    </tr>
                    <?php if (!empty($petres_name)) { ?>
                        <tr align="center" style="color: blue">
                            <th><?php if ($petres_name['pno'] <= 1) echo strtoupper(trim($petres_name['pet_name']));
                        ...
                        <tr align="center">
                            <th style="color: black">Versus !!!!!</th>
                        </tr>

                        <tr align="center" style="color: blue">
                            <th><?php if ($petres_name['rno'] <= 1) echo strtoupper(trim($petres_name['res_name']));
...
<?php
    ...
    public function additional_address_modify()
    {
        $diary_no = session()->get('filing_details')['diary_no'];
        if (empty($diary_no)) {
            echo 'additional address modify diary is required';
            exit();
        }
        ...
        if (!empty($additional_address)) {
            $result = '';
            $data['dist_list'] = array();
            $sno = 0;
            foreach ($additional_address as $row) {
                $data['sno'] = $sno;
                $data['row1'] = $row;
                if (!empty($row['state'])) {
                    $data['dist_list'] = $this->DropdownListModel->get_districts_list($row['state']);
                }
    ...
    public function delete_additional_address()
    {
        ...
        if (empty($hd_main_idP)) {
            echo 'additional address id is required';
            exit();
        }
        ...
        if ($is_party_additional_address) {
            ...
            echo "Data is mot delete please try again !!!";
        }
        exit();
    }
