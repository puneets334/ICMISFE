...
class Diary extends BaseController
{
    ...
    public $Model_main_a;
    public $Model_diary;
    public $ModelCntDiaryNo;
    public $ModelCasetype;
    ...
    public function search()
    {
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (!isset($_REQUEST['redirect_url'])){$_REQUEST['redirect_url']=base_url('Filing/DiaryModify');}
            ...
            if ($search_type=='D'){
                ...
                if (!empty($diary_no)){
                    $get_main_table= $this->DropdownListModel->get_diary_details_by_diary_no($diary_no);
                }else{$get_main_table=array();}
            }
            if ($get_main_table){
                $this->session->set(array('filing_details'=> $get_main_table));
                echo '1@@@'.$redirect_url;exit();
                //return redirect()->to('Filing/Diary/redirect_on_diary_user_type');exit();
            }else{
                echo '3@@@Data not found!';exit(); //session()->setFlashdata("message_error", 'Data not Fount');
            }
            exit();
        }
    ...
    function redirect_on_diary_user_type() {
        if(session()->get('login')) {
            return redirect()->to('Filing/DiaryModify');
        }else{
    ...
    public function index()
        ...
        $data['court_type_list'] = $this->DropdownListModel->get_court_type_list();
        $data['usersection']=$this->DropdownListModel->get_usersection();
        ...
        $data['casetype']=$this->DropdownListModel->get_case_type($role);
        $data['casetype_nature_sci']=$this->DropdownListModel->get_case_type($role,'nature_sci');
        $data['role']=$role;
...
public function get_diary_info(){
    ...
    if ($cause_title && !empty($cause_title)){
        foreach ($cause_title as $row){
            ...
            if ($row['c_status']=='P'){$c_status='Case Status : <span class="text-primary">Pending</span><br/>';}elseif ($row['c_status']=='D'){ $c_status='Case Status : <span class="text-danger">Disposed</span></span><br/>';}
        ...
        if(!empty($is_renewed)){ ?><br> <span style="text-align: center;font-size: 20px;color:green"><?php echo "Already Renewed";?></span>
            ...
            if(count($get_new_cav)>0){?>
                <table class="table" bgcolor="#ffe4c4" border="1"> <th>Old Caveat No.</th><th>Renewed Caveat No.</th><th>Renewed On</th>
                    <tr><td><?php echo $get_new_cav[0]['old_caveat_no']; ?></td><td><?php echo $get_new_cav[0]['new_caveat_no']; ?></td><td><?php echo $get_new_cav[0]['renew_date']; ?></td></tr>
                </table>
            <?php } } } else { echo "<font style='text-align: center;font-size: 14px;color: red'>Case not found</font>"; }
exit();
}
public function get_diary_delete(){
    ...
    if ($cause_title && !empty($cause_title)) {
        foreach ($cause_title as $row) {
            ...
            if(!empty($is_renewed)){ echo'<span style="text-align: center;font-size: 20px;color:green">Already Renewed</span>'; exit();}
    ...
    if ($data_array) {
        ...
        $final_array = array_merge($data_log, $data_array[0]);
       // echo '<pre>';print_r($final_array);exit();
        ...
        echo "<font style='text-align: center;font-size: 14px;color: red'>Please Contact Computer cell $disposed.</font>"; exit(); }
}
    ...
    function get_adv_name() {
        ...
        $adv = $this->DropdownListModel->get_adv_name($p_r_advt,$advno,$advyr,$state_id);
    ...
    public function save_new_filing(){
       ...
       /* if (isset($_REQUEST['type_special']) && $_REQUEST['type_special']==6){
        ...
        if (!isset($_REQUEST['hd_r_barid'])){$radvno_and_yr=$_REQUEST['hd_r_barid']=0;}else{if (empty($radvno_and_yr)){ $radvno_and_yr=$_REQUEST['hd_r_barid']=0;}}
        ...
        if($_REQUEST['controller']=='I') {
            echo '!~!';
            //error_reporting(0);
            $fil_q= $this->ModelCntDiaryNo->select('max_diary_no')->where('diary_no_year',$year)->get()->getRowArray();
            ...
            if ($_REQUEST['st_status'] == '0') {
                ...
                $da = 0;
                $res_nt = $this->ModelCasetype->select('nature,casename')->where('casecode', $_REQUEST['ddl_nature'])->get()->getRowArray();
    ...
    $is_special_category_filing=$this->Model_special_category_filing->insert($insert_special_category_filing);
            ...
            <table align="center" width="100%" style="text-align: center;">
                ...
                if($check_if_fil_user==1 && !empty($alloted_to)){
                    ?>
                    <tr><th style="color: #ff6600;font-size: 16px;">File Automatically Allotted to: <?php if (isset($alloted_to[1]) && !empty($alloted_to[1])){ echo $alloted_to[1] ;} if (isset($alloted_to[0]) && !empty($alloted_to[0])){ echo ' ['.$alloted_to[0].']'; }?></th></tr>
                    <?php
                }
                if(!empty($alloted_to)){
                    ?>
                    <tr><th style="color: #ff6600;font-size: 16px;"><?php if (isset($alloted_to[2]) && !empty($alloted_to[2])){ echo $alloted_to[2];}?></th></tr>
                    <?php
                }
                ?>

                <tr align="center">
                    <th>Diary No.:<h2 style="color: blue"><?php echo $fil.'/'.$year;?></h2></th></tr>

                <tr align="center" ><th style="font-size: 17px;color: blue"><?php echo $pet_cause_title; ?></th></tr>
                <tr align="center" ><th style="font-size: 14px;color: blue">Versus</th></tr>
                <tr align="center" ><th style="font-size: 17px;color: blue"><?php echo $res_cause_title; ?></th></tr><br/>
                <tr align="center" ><th style="font-size: 17px;color: blue"><?php echo "Case Type: ".$res_nt['casename']; ?></th></tr><br/>
                <tr align="center" ><th style="font-size: 17px;color: blue"><?php echo "Filed by: ".strtoupper(trim($_REQUEST['padvname']))."(ADV)"; ?></th></tr><br/>
                <?php
                ...
                if (!empty($_REQUEST['ddl_st_agncy']) && !empty($_REQUEST['ddl_bench'])) {
                    ...
                    if ($ref_agency_code_details && !empty($ref_agency_code_details)){
                        echo "<tr aligh='center'><th style='font-size: 17px;color: blue'>Bench: ".strtoupper(trim($ref_agency_code_details['agency_state']))  .' - '.strtoupper(trim($ref_agency_code_details['agency_name']))."</th></tr><br>";
                    }
                    ?>
...
public function additional_address(){
    ...
    public function get_sc_parties()
    {
        ...
        if (empty($diary_no)){ $response='No Record Found!!'; exit();}
