...
use App\Models\Common\Component\CaseStatusModel;
...
class Tagging extends BaseController
{
    ...
    function __construct(){
       ...
       $this->DropdownListModel = new DropdownListModel();
        $this->CaseStatusModel = new CaseStatusModel();
    ...
    public function index()
    {
        ...
        if(empty($_SESSION['filing_details']['diary_no'])){
           // session()->setFlashdata("message_error", 'Please enter diary number or case number');
            return redirect()->to('Filing/Diary/search');exit();
            exit;
        }
        $diary_no = $sessionData['filing_details']['diary_no'];

        $data['section'] = '';
        $data=array(); $conct_data_diary=array();$diary_details_connected = array(); $all_connected_cases=$connected_cases=array(); $list_not_before = $list_not_bf= array(); $hearing_details=$heardt_details = array();$data['old_category_name']='';
        ...
        if($diary_details_connected['ccdet'] != 'NA'){
        ...
        $old_category_data = json_decode($this->CaseStatusModel->get_old_category($diary_no, $is_archived_flag),true);
    ...
    public function conn_case_status(){
        ...
        $list_not_bf['not_before'] = $list_not_before;
        $heardt_details['heardt'] = $hearing_details;
        $old_category_data = json_decode( $this->CaseStatusModel->get_old_category($diaryno_conn, ''),true);
