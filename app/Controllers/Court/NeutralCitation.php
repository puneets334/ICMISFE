...
use App\Models\Court\NeutralCitationModel;
...
class NeutralCitation extends BaseController
{
    ...
    function __construct()
    {
        ...
        $this->model = new NeutralCitationModel();
    ...
    public function index()
    {
        ...
        if ($this->request->getPost('diary_number')) {
            if ($this->request->getPost('insert')) {
                ...
                if ($check_digital_sign == 0) {
                    ...
                    $data['insert'] = @insert('public.neutral_citation', $array);
        ...
        return view('Court/NeutralCitation/neutral_citation', $data);
    }
    ...
    {                   // generate QR Code on new Judgment
        ...
        $number = uniqid();
        $desired_dir = "/home/reports/supremecourt/qr_judgments/" . $number;
        //$desired_dir = "../reports/supremecourt/qr_judgments/".uniqid();
        if (!empty($this->request->getFiles('file'))) {

            $getFile = $this->request->getFiles('file');

            $file_name = $getFile['file']->getName();
            $file_size = $getFile['file']->getSize();
            $file_tmp = $getFile['file']->getTempName();
            $file_type = $getFile['file']->getMimeType();
    ...
    private function generateQR($neutral_citation_number, $file_name, $desired_dir)
    {
        ...
        if (is_dir($desired_dir . "/judgment_after_qr") == false) {
            mkdir($desired_dir . "/judgment_after_qr", 0755, true);
        }
    ...
    public function delete()
    {
        ...
        return view('Court/NeutralCitation/neutral_citation_delete', $data);
    }
