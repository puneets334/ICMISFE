...
//        array(12) { ["searchBy"]=> string(1) "p" ["fromDate"]=> string(0) "" ["toDate"]=> string(0) ""
//        ["dealingSection"]=> string(1) "0" ["caseType"]=> string(1) "0" ["caseNo"]=> string(0) ""
//        ["caseYear"]=> string(4) "2024" ["diaryNumber"]=> string(0) "" ["diaryYear"]=> string(4) "2024"
//        ["processId"]=> string(5) "12345" ["processYear"]=> string(4) "2024" ["status"]=> string(4) "9999" }
...
//        epd.ref_letter_status_id in (4,5)>> and epd.process_id=12345 and process_id_year=2024 and epd.ref_postal_type_id=
    ...
    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>END OF DISPATCH AD TO SECTION REPORT<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */

    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW FUNCTIONS ARE OF Dispatch to Officer/Section <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */
...
//                echo ">>>".$whereCondition."LLL".$receiptModeCondition;
//                die;
//                >>> and ept.dispatched_to_user_type='o' and ept.dispatched_to=1828LLL and ecpd.ref_postal_type_id=5
    ...
    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>END OF Dispatch to Officer/Section REPORT<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */

    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW FUNCTIONS ARE OF Update Serve Status <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */
...
//        epd.ref_letter_status_id in (3,6)>> and epd.process_id=134 and process_id_year=2024
        ...
        $dataToUpdateServeStatus = $this->RIModel->enteredDakToDispatchInRIWithProcessId($wherePostalDispatch, $whereDateRange);
...
//        extract($_GET);
        $stage = $_POST['stage'];
        $id = $_POST['id'];
        $serveType = $this->RIModel->getServeType(2,$stage);
...
//        extract($_POST);
    ...
    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>END OF Update Serve Status REPORT<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */

    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW FUNCTIONS ARE OF  Receipt By Section Officer <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */
...
//            print_r($userDetails);
//            die;
//            echo $userDetails[0]['section'];
//            die;
            if(isset($fromDate) && isset($toDate)){
                $data['reportType']=$reportType;
                $data['fromDate']=$fromDate;
                $data['toDate']=$toDate;
                $data['receivedData'] = $this->RIModel->getReceivedByConcernDateWise($this->common->date_formatter($fromDate,'Y-m-d'),$this->common->date_formatter($toDate,'Y-m-d'),$reportType,$usercode,$userDetails[0]['section']);
    ...
    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>END OF  Receipt By Section Officer REPORT<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */

    /* >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>BELOW FUNCTIONS ARE OF Received Query <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  */
...
//        }
    ...
    public function getDakDataForReceive(){
        ...
        $userDetails = $this->RIModel->getUserDetails($usercode);
        $data['dealingSections'] = $this->RIModel->getSection();
        $data['forReceiveInSection']=$this->RIModel->getDakDataForReceive($usercode,$userDetails[0]['section'],'P');
        $data['forInitiatedReceivedInSection']=$this->RIModel->getInitiatedDakDataForReceive($usercode,$userDetails[0]['section'],'P');
        //        echo "<pre>";   print_r($data);      die;
        $this->db->transComplete();
