...
class Ajaxcalls extends BaseController
{
    ...
    public function getAddressByPincode()
    {
        ...
        $address = $this->DropdownListModel->getPincodeDetails($pincode);
        echo json_encode($address);
    }
    public function getSelectedState()
    {
        $states = $this->DropdownListModel->get_address_state_list();
        $stateIdName = array();
    ...
    public function get_address_state_list()
    {
        ...
        $states = $this->DropdownListModel->get_address_state_list($stateId);
    ...
    public function get_districts()
    {
        ...
        $districts = $this->DropdownListModel->get_districts_list($state_id);
        $dropDownOptions = '<option value="">Select District</option>';
    ...
    public function getSelectedDistricts()
    {
        ...
        $stateId = sanitize($_GET['state_id']);
        $districts = $this->DropdownListModel->get_districts_list($stateId);
    ...
    public function get_subcategories()
    {
        ...
        if (!empty($main_category_id) && !empty($main_category_id)) {
            $sub_categories = $this->DropdownListModel->get_subcategory_list($main_category_id);
            if (!empty($sub_categories)) {
                foreach ($sub_categories as $sub_category) {
                    ...
                    $dropDownOptions .= '<option value="' . sanitize($sub_category['id']) . '" ' . $selected_subcat_text . '>' . sanitize(strtoupper($sub_category['sub_name4'])) . '</option>';
                }
            } else {
    ...
    public function get_aor_name()
    {
        $q = strtolower($_GET["term"]);
        if (!$q) return;
        // $sql = "select aor_code,name from master.bar where isdead='N' and if_sen='N' and (name ilike '%".$q."%' or aor_code ilike '%".$q."')";
    ...
    function get_police_station_list()
    {
        ...
        if (!empty($state_agency) && !empty($district_id)) {
            ...
            $policeStations = $this->DropdownListModel->get_police_station_list($state_agency, $district_id);
    ...
    function get_case_types()
    {
        ...
        if (!empty($state_agency) && !empty($court_type)) {
            $caseTypes = $this->DropdownListModel->get_case_type_court($state_agency, $court_type);
            ...
            foreach ($caseTypes as $case_type) {
                if ($court_type == 4) {
                    ...
                    $dropDownOptions .= '<option value="' . sanitize($case_type['casecode']) . '">' . sanitize(strtoupper($case_type['casename'])) . '</option>';
                } else {
                    if (!empty($_POST['case_type'])) {
    ...
    function get_judges_list()
    {
        $state_agency = (sanitize($_POST['state_agency']));
        $court_type = $_POST['court_type'];
        $dropDownOptions = '<option value="">Select Judge</option>';
        if (!empty($state_agency) && !empty($court_type)) {
            $all_judges = $this->DropdownListModel->get_all_judges($state_agency, $court_type);
    ...
    function get_rto_code()
    {
        ...
        if (!empty($state_agency)) {
            $all_rto_code = $this->DropdownListModel->get_all_rtocode($state_agency);
    ...
    function get_m_from_court_list()
    {
        ...
        if (!empty($court_type)) {
            $all_types_list = $this->DropdownListModel->get_all_court_type_list($court_type);
    ...
    function get_states_list()
    {
        ...
        $all_states_list = $this->DropdownListModel->icmis_states();
    ...
    public function get_bench()
    {
        ...
        $dropDownOptions = '<option value="">Select High Court Bench</option>';
        if (!empty($high_court_id) && !empty($court_type)) {
            $hc_benches = $this->DropdownListModel->getHighCourtData($params);
            foreach ($hc_benches as $bench) {
