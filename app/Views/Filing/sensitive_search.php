            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <ul class="nav-breadcrumb">
                        <li>
                            <?php
                            if (($uri->getSegment(2) == 'SensitiveInfo' && $uri->getSegment(3) != 'report')){
                                $ColorCode = 'background-color: #01ADEF';
                                $status_color = 'btn-primary';
                                $url_sensitive_info = base_url('Filing/SensitiveInfo');
                            }else{
                            ...
                            ?>
                            <a href="<?= $url_sensitive_info; ?>"><button  class="btn btn-block <?php echo $status_color; ?>">Sensitive Information</button> </a>
                        </li>
                        ...
                        <li>
                            ...
                            if (($uri->getSegment(2) == 'SensitiveInfo' && $uri->getSegment(3) == 'report')) {
                                $ColorCode = 'background-color: #01ADEF';
                                $status_color = 'btn-primary';
                                $url_sensitive_report = base_url('Filing/SensitiveInfo/report');
                            } else{
                                $ColorCode = 'background-color: #169F85;color:#ffffff;';
                                $status_color = 'btn-outline-primary';
                                $url_sensitive_report = base_url('Filing/SensitiveInfo/report');
                            ...
                            ?>
                            <a href="<?= $url_sensitive_report; ?>"><button type="button" class="btn btn-block <?php echo $status_color; ?>"> Sensitive Report</button> </a>
            ...
            $('#sensitive_search').on('submit', function () {
                ...
                if ($('#sensitive_search').valid()) {
                    ...
                    if(validateFlag){
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url('Filing/SensitiveInfo/get_details'); ?>",
                            data: form_data,
                            beforeSend: function () {
            ...
            if(case_diary.length != 0) {
                if(case_info.length == 0)
                {
                    ...
                    if(validateFlag) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "<?php echo base_url('Filing/SensitiveInfo/update_case'); ?>",
                            cache: false,
                            async: true,
