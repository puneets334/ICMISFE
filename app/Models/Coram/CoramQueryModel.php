...
class CoramQueryModel extends Model
{
    ...
    public function get_coram_detail_data($diary_no)
    {
        ...
        $builder1->select("j.jcode,string_agg(concat(j.jname,'[',j.abbreviation,']'),'') as jname,c.diary_no,'C' AS notbef,c.ent_dt,n.res_add, CAST(c.jud AS TEXT) as coram,j.judge_seniority,c.usercode,c.display");
        ...
        $builder2->select("j.jcode,string_agg(concat(j.jname,'[',j.abbreviation,']'),'') as jname,h.diary_no,'C' AS notbef,h.ent_dt,n.res_add, CAST(h.coram AS TEXT),j.judge_seniority,h.usercode, 'Y' as display");
        ...
        $builder4->select("j.jcode,concat(j.jname,'[',j.abbreviation,']') as jname,n.diary_no,n.notbef,n.ent_dt,nbs.res_add, CAST(0 AS TEXT),j.judge_seniority,n.usercode, 'Y' as display");
        ...
        $builder5->select("j.jcode,concat(j.jname,'[',j.abbreviation,']') as jname,n.diary_no,n.notbef,n.ent_dt,nbs.res_add, CAST(0 AS TEXT),j.judge_seniority,n.usercode, 'N' as display");
    ...
    public function get_coram_entry_date($diary_no, $coram)
    {
        ...
        $final_query = $this->db->query($sql1 . " UNION " . $sql2 . " UNION " . $sql3 . " ORDER BY ent_dt ASC");
        $query = $final_query;
