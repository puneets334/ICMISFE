...
class ModelScefmMatters extends Model
{
    public function get_all_matters_ib()
    {
        ...
        $query = $this->db->table('public.main m');
        ...
        $query->select("TO_CHAR(diary_no_rec_date, 'DD-DD-YYYY') AS diary_date");
        ...
        $query->join('public.efiled_cases ec', 'm.diary_no = ec.diary_no AND efiled_type = \'new_case\' AND ec.display = \'Y\'');
        ...
        $query->join('public.heardt h', 'm.diary_no = h.diary_no','left');
        $query->join('public.special_category_filing s', 'ec.diary_no = s.diary_no AND s.display = \'Y\'','left');
        ...
        $query->where('(h.diary_no IS NULL OR DATE(m.diary_no_rec_date) >= \'2023-09-20\')');
        $query->where('DATE(ec.created_at) >', '2023-07-29');
    ...
    public function transfer_case($case_type,$diary_no)
    {
        ...
        if(in_array($case_type,$crc)) {
            $dacode=''; $section_id='';
                $result_lowerct = is_data_from_table('public.lowerct', ['diary_no' => $diary_no,'is_order_challenged' => 'Y','lw_display' => 'Y'],'lct_casetype,lct_caseno,lct_caseyear','R');
              if (empty($sql_challenged_case)){
                  $result_lowerct = is_data_from_table('public.lowerct_a', ['diary_no' => $diary_no,'is_order_challenged' => 'Y','lw_display' => 'Y'],'lct_casetype,lct_caseno,lct_caseyear','R');
              }
           if (!empty($result_lowerct)){
               ...
               if (!empty($result_get_dno)) {
                   ...
                   $result_da = is_data_from_table('public.main', ['diary_no' => $lower_ct_diary_no],'dacode,section_id','R');
                   if (empty($result_da)){
                       $result_da = is_data_from_table('public.main_a', ['diary_no' => $lower_ct_diary_no],'dacode,section_id','R');
                   }
                   if (!empty($result_da)){
            ...
            $is_efiled_cases_transfer_status=update('public.efiled_cases_transfer_status',$update_efiled_cases_transfer_status,['diary_no'=>$diary_no]);
            ...
            if ($is_efiled_cases_transfer_status) {
                // to do from here
                $query_marking = is_data_from_table('public.mark_all_for_hc', ['display' => 'Y']);
                ...
                $is_insert_fil_trap = insert('public.fil_trap', $insert_data_fil_trap);
                if ($is_insert_fil_trap) {
                $select_fil_trap_seq = is_data_from_table('public.fil_trap_seq', ['ddate' => $curdate, 'utype' => 'DE', 'user_type' => $fil_type]);
                if (empty($select_fil_trap_seq)) {
                    ...
                    $is_insert_fil_trap_seq = insert('public.fil_trap_seq', $insert_fil_trap_seq);
                } else {
                    $update_fil_trap_seq = [
                        'no' => (!empty($check_ava_row['to_userno'])) ? $check_ava_row['to_userno'] : 0,
                        'updated_on' => date("Y-m-d H:i:s"),
                    ...
                    $is_update_fil_trap_refil_users = update('public.fil_trap_seq', $update_fil_trap_seq, ['ddate' => $current_date, 'utype' => $utype, 'user_type' => $fil_type]);
                }
            }
    ...
    public function check_if_SCR_available_with_fil_trap_seq($fil_type,$usertype,$utype)
    {
        $query = $this->db->table('public.fil_trap_users a')
            ...
            ->join('public.fil_trap_seq c', 'c.no < b.empid', 'left')
