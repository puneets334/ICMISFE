...
class CoramModel extends Model
{
    ...
    public function coram_detail($diary_no){
        ...
        $builder1->select("j.jcode,string_agg(concat(j.jname,'[',j.abbreviation,']'),'') as jname,c.diary_no,'C' AS notbef,c.ent_dt,n.res_add, CAST(c.jud AS TEXT) as coram,j.judge_seniority,c.usercode");
        ...
        $builder2->select("j.jcode,string_agg(concat(j.jname,'[',j.abbreviation,']'),'') as jname,h.diary_no,'C' AS notbef,h.ent_dt,n.res_add, CAST(h.coram AS TEXT),j.judge_seniority,h.usercode");
        ...
        $builder3->select("j.jcode,concat(j.jname,'[',j.abbreviation,']') as jname,n.diary_no,n.notbef,n.ent_dt,nbs.res_add, CAST(0 AS TEXT),j.judge_seniority,n.usercode");
    ...
    public function not_before_judge($diary_no){
        ...
        $builder->join("not_before g", "g.diary_no = a.diary_no AND d.jcode = g.j1", "left");
