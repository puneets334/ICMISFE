...
public function filing_stats()
{
    ...
    $sql_filing = "INSERT INTO filing_stats(filing_date, updation_time, total_filed, old_efiled, new_efiled, physical_filed, registered, refiled, checked_verified, verified, tagging_verification, verification_refiled_total, verification_refiled_reg, filing_alloted, filing_completed, filing_pending, refiled_alloted, refiled_completed, refiled_pending, create_modify, updated_on, updated_by, updated_by_ip)
                   SELECT NOW(), '$cur_time',
                          SUM(filed) AS filed,
                   ...
                   FROM (
                       ...
                       FROM (
                           ...
                           FROM main m
                           LEFT JOIN efiled_cases ec ON ec.diary_no = m.diary_no AND efiled_type = 'new_case'
                       ...
                       SELECT 0 AS filed,
                              ...
                              0 AS refil_rem
                       FROM obj_save
                       WHERE DATE(rm_dt) = CURDATE() AND display = 'Y'
                       ...
                       SELECT 0 AS filed,
                              ...
                              0 AS registered,
                              0 AS refiled,
                              SUM(CASE WHEN DATE(verification_date) = CURDATE() THEN 1 ELSE 0 END) AS checked_verified,
                              SUM(CASE WHEN verification_status = '0' AND DATE(verification_date) = CURDATE() THEN 1 ELSE 0 END) AS verified,
                              SUM(CASE WHEN verification_status = '1' AND c_status = 'P' THEN 1 ELSE 0 END) AS tag_verified,
                              0 AS verification_re_filing,
                              0 AS verification_re_filing_reg,
                              0 AS fil_allot,
                              0 AS fil_comp,
                              ...
                              0 AS refil_comp,
                              0 AS refil_rem
                       FROM defects_verification d
                       LEFT JOIN main m ON d.diary_no = m.diary_no
                       ...
                       FROM (
                           SELECT os.diary_no,
                                  m.fil_dt,
                                  NULLIF(MIN(COALESCE(rm_dt, '1949-12-31')), '1949-12-31') AS min_rm_dt
                           FROM obj_save os
                           JOIN main m ON m.diary_no = os.diary_no
                           LEFT JOIN heardt b ON os.diary_no = b.diary_no
                           WHERE ((b.diary_no IS NULL)
                             ...
                             AND c_status = 'P'
                             AND os.display = 'Y'
                           GROUP BY os.diary_no, m.fil_dt
                       ) temp
                       WHERE min_rm_dt IS NOT NULL
                       ...
                       FROM (
                           SELECT COUNT(DISTINCT CASE WHEN remarks LIKE 'DE -> SCR' THEN diary_no END) AS fil_allot,
                           ...
                           FROM (
                               SELECT diary_no,
                                      ...
                                      comp_dt
                               FROM fil_trap
                               WHERE (remarks LIKE 'DE -> SCR' OR remarks LIKE 'FDR -> SCR')
                                 AND DATE(disp_dt) = CURDATE()
