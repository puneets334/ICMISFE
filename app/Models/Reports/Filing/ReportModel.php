...
";
    ...
    function get_complete_filing_report($report_date, $report_for){
        ...
        $sql1 = "select sum(e_filed)e_filed, sum(counter_filed)counter_filed from(
                SELECT SUM(CASE WHEN (ack_id!=0) THEN 1 ELSE 0 END) e_filed, SUM(CASE WHEN ( ack_id=0) THEN 1 ELSE 0 END) counter_filed 
                FROM main 
                where DATE(diary_no_rec_date) = '$report_date'
                union   
                SELECT SUM(CASE WHEN (ack_id!=0) THEN 1 ELSE 0 END) e_filed, SUM(CASE WHEN ( ack_id=0) THEN 1 ELSE 0 END) counter_filed 
                FROM main_a 
                where DATE(diary_no_rec_date) = '$report_date')efiled_Cases";
        ...
        $sql2 = "select count(diary_no) refiled from(
                select distinct diary_no FROM obj_save WHERE DATE(rm_dt) = '$report_date' AND display='Y' GROUP BY diary_no
                union 
                select distinct diary_no FROM obj_save_a WHERE DATE(rm_dt) = '$report_date' AND display='Y' GROUP BY diary_no
                )a ";
        $result = $db->query($sql2)->getRowArray();
        $result_array['refiled'] = $result['refiled'];

        $sql3 = "select sum(total)total, sum(veri)veri from(    
                 SELECT count(*) total,SUM(CASE verification_status WHEN '0' THEN 1 ELSE 0 END)veri FROM defects_verification
                 WHERE date(verification_date) ='$report_date'
                 union 
                 SELECT count(*) total,SUM(CASE verification_status WHEN '0' THEN 1 ELSE 0 END)veri FROM defects_verification_a
                 WHERE date(verification_date) ='$report_date')b";
        $result = $db->query($sql3)->getRowArray();
        $result_array['verified'] = $result['veri'];
        $result_array['t_crawl_veri'] = $result['total'];

        $sql3 = "SELECT count(distinct a.diary_no) pending_tagging
        ...
        $sql = "SELECT COUNT(*) tt,SUM(CASE WHEN fil_dt is not null THEN 1 ELSE 0 END) reg
                FROM (
...
group by date1; ";
            ...
            and dc.display='Y' group by dc.usercode,u.name,u.empid
            UNION
            select dc.usercode,name,empid,count(1) as documents from docdetails_a dc left join 
            master.users u on dc.usercode=u.usercode where date(ent_dt) between '$first_date' and '$to_date'
            and dc.display='Y' group by dc.usercode,u.name,u.empid)a group by usercode, name, empid";
        }
