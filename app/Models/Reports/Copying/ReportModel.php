...
class ReportModel extends Model
{
    ...
    function getEcopyStatus($data)
    {
        ...
        IFNULL(SUM(b.copying_fee_in_stamp), 0) AS copying_fee_in_stamp, IFNULL(SUM(b.copying_service_charges), 0) AS copying_service_charges,
        IFNULL(SUM(b.postage), 0) AS postage,
        ...
        (SELECT
            ...
            a.copy_category,
            a.delivery_mode,
            SUM(CASE WHEN b.payment_type_id = 9527 THEN b.amount ELSE 0 END) AS copying_fee_in_stamp,
            SUM(CASE WHEN b.payment_type_id = 9528 THEN b.amount ELSE 0 END) AS copying_service_charges,
            SUM(CASE WHEN b.payment_type_id = 9525 THEN b.amount ELSE 0 END) AS postage
          FROM
            copying_order_issuing_application_new a
          LEFT JOIN
            dblink('dbname=e_services', 'SELECT payment_type_id, amount, order_batch_merchant_batch_code FROM bharat_kosh_request_batch') AS b(id INT, amount TEXT, order_batch_merchant_batch_code TEXT)
          ON
    ...
    function getEpay($data)
    {
        ...
        FROM
            dblink('dbname=e_services', 'SELECT order_code, response_status, entry_time FROM bharat_kosh_status') AS s(order_code TEXT, response_status TEXT, entry_time TIMESTAMP)
        INNER JOIN 
            dblink('dbname=e_services', 'SELECT order_code, shipping_first_name, shipping_last_name, order_batch_total_amount FROM bharat_kosh_request') AS r(order_code TEXT, shipping_first_name TEXT, shipping_last_name TEXT, order_batch_total_amount NUMERIC) ON s.order_code = r.order_code 
        LEFT JOIN 
            dblink('dbname=e_services', 'SELECT cart_description, amount, order_batch_merchant_batch_code FROM bharat_kosh_request_batch') AS b(cart_description TEXT, amount TEXT, order_batch_merchant_batch_code TEXT) ON b.order_batch_merchant_batch_code = r.order_code
        WHERE 
