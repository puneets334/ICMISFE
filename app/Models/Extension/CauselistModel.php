...
class CauselistModel extends Model
{
    ...
    function getListingDates()
    {
        ...
        $query = $this->db->table('public.heardt')
            ->where('mainhead', 'M')
    ...
    function get_data($data)
    {
        ...
        if ($data['received'] != 0) {
            if ($data['received'] == 1)
                $received_condition = " AND (remarks='SCN -> IB-Ex' and r_by_empid!=0)";
            else if ($data['received'] == 2)
                $received_condition = " AND ((remarks='SCN -> IB-Ex' and r_by_empid=0) OR (remarks!='SCN -> IB-Ex'))";
        }
        ...
        $sql = "select j.* from(select distinct cct.ent_dt conct_ent_dt,i.file_id,cl.id as is_printed,r.courtno, lp.purpose,
                                ...
                                where h.mainhead = '$data[main_regular]' and h.next_dt = '$data[listing_date]' $court_condition $board_condition $main_condition $ma_condition $section_condition $received_condition $scanned_condition
