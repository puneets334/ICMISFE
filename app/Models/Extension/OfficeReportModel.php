...
class OfficeReportModel extends Model
{
    ...
    public function record_for_table($fromDate, $toDate, $user_code = '')
    {
        ...
        $builder1 = $this->db->table("public.office_report_details");
        $builder1->select("web_status, office_report_details.display, office_report_details.diary_no, reg_no_display, 
                      CASE WHEN case_nature = 'R' THEN 'Criminal' ELSE 'Civil' END AS case_nature, r_nature, rec_dt, order_dt, 
                      office_repot_name, DATE(rec_dt) AS d, summary, office_report_details.office_repot_name");
        $builder1->join('master.office_report_master', 'office_report_details.office_report_id=office_report_master.id', 'left');
        $builder1->join("public.main", "office_report_details.diary_no=main.diary_no", 'left');
        ...
        $builder2 = $this->db->table("public.office_report_details_a as orda");
        $builder2->select("web_status, orda.display, orda.diary_no, reg_no_display, 
                      CASE WHEN case_nature = 'R' THEN 'Criminal' ELSE 'Civil' END AS case_nature, r_nature, rec_dt, order_dt, 
                      office_repot_name, DATE(rec_dt) AS d, summary, orda.office_repot_name");
        $builder2->join('master.office_report_master', 'orda.office_report_id=office_report_master.id', 'left');
        $builder2->join("public.main_a", "orda.diary_no=main_a.diary_no", 'left');
    ...
    public function discard_data($diary_no, $rec_date)
    {
        ...
        $builder = $this->db->table('public.office_report_details');
    ...
    public function check_heardt($diary_no)
    {
        ...
        $builder->where("next_dt >= '2023-10-01'");
        $query = $builder->get();
        //        $query=$this->db->getLastQuery();echo (string) $query."<br>";exit;
        $result = $query->getResultArray();
        //        echo "<pre>";
    ...
    {
        $builder = $this->db->table("public.conct");
    ...
    public function filing_details($diary_no)
    {
        ...
        $builder->select("
            CASE 
                WHEN (active_casetype_id IS NULL OR active_casetype_id = 0) 
                THEN casetype_id 
            ...
            lastorder
        ");
        $builder->join('master.casetype b', 'a.casetype_id = b.casecode', 'left');
        $builder->where("diary_no", $diary_no);
        $builder->where("display", 'Y');
        $query = $builder->get();
    ...
    public function check_listed($diary_no)
    {
        ...
        // Return the result if found, otherwise return 0
        if ($query->getNumRows() > 0) {
            return $result;
        } else {
            return 0;
    ...
    public function check_ofc_report_detail_publish($diary_no, $ddl_ord_date)
    {
        ...
        $builder->where('order_dt', $ddl_ord_date);
        $builder->where('display', 'Y');
        $query = $builder->get();
        $result = $query->getResultArray();
    ...
    {
        // Ensure $diary_no is properly escaped to prevent SQL injection
        $diary_no = $this->db->escape($diary_no);

        // Raw SQL query
        $sql = "
        SELECT 
            CONCAT(name, ' ', COALESCE(CONCAT(p.pet_res, ' [', p.sr_no_show, ']'), '')) AS name,
            c.caveat_no,
            TO_CHAR(d.ent_dt, 'DD-MM-YYYY') AS rec_dt
        FROM public.caveat_diary_matching cd
        LEFT JOIN public.caveat c ON cd.caveat_no = c.caveat_no
        JOIN master.bar ON c.pet_adv_id = bar.bar_id
        LEFT JOIN public.docdetails d ON cd.diary_no = d.diary_no AND d.doccode = 18 AND d.display = 'Y'
        LEFT JOIN public.caveat_party cp ON cd.caveat_no = cp.caveat_no AND cp.pet_res = 'P'
        LEFT JOIN public.party p ON cd.diary_no = p.diary_no AND p.partyname LIKE CONCAT('%', cp.partysuff, '%')
        WHERE c.c_status = 'P' AND cd.diary_no = $diary_no";
    ...
    public function check_partystuff($caveatno)
    {
        ...
        $builder = $this->db->table("public.caveat_party");
    ...
    {
        $sql = "select  concat(pet_res,' [',sr_no_show,']') from public.party where diary_no=$diary_no and partyname like '%$partyname%'";
    ...
    public function check_amt_query($diary_no)
    {
        ...
        $builder->join('public.mul_category c', 'm.diary_no=c.diary_no');
...
where diary_no=$res_linked_case and b.display='Y'";
    ...
    function save_revised_records($year, $diary_no, $sp_listed_on, $hd_or_id, $ggg, $str, $dairy_no, $ucode, $ddl_rt, $batch, $new_file_name, $summary = null)
    {
        ...
        if ($fwrite == true) {
            ...
            $builder5 = $this->db->table("public.office_report_details");
            ...
            if (!empty($res_sel)) {
                ...
                if ($res_sel <= 0) {
                    ...
                    if (file_exists($fil_nm)) {
                        ...
                        $builder9 = $this->db->table("public.office_report_details");
                    ...
                    $builder6 = $this->db->table("public.office_report_details");
    ...
    public function save_office_report($dataArr)
    {
        ...
        if (empty($rs_previous_id1)) {
            ...
            $builder1 = $this->db->table("public.docdetails");
 ...
 judgement_covered_in,lct_judge_desg ");
    ...
    public function get_dismissal_type($diary_no)
    {
        ...
        $builder = $this->db->table("public.dispose a");
