...
class LeaveGrantDisposeModel extends Model{
        ...
        if($dn!=""){
            ...
            $result_p1 = $query1->getResultArray();
             if(count($result_p1) > 0){
                ...
                $builder2->select(['diary_no','list',"CASE WHEN conn_key = diary_no THEN 'M' ELSE conn_type END AS c_type"]);
                ...
                foreach ($me2 as $row => $link) {
                    ...
                    if ($t_list1[1] != '' && $linkm['c_status'] != "D") {
                        $check_for_cb = "yes";
                        $check_for_reg = get_cases_mf($link['diary_no']); 
                        
                        $t_var = explode(",", $check_for_reg);
                        $t_print = ($t_var[0] == 'yes') ? "(D-Stage)" : "";
                    
                        $t_lc = 0;
                        $t_lcid = 0;
                        $sql_l = "SELECT count(*) as cnt, GROUP_CONCAT(lower_court_id) as lcid FROM `lowerct` WHERE `diary_no` = '" . $link['diary_no'] . "' AND lw_display='Y' AND is_order_challenged='Y' GROUP BY diary_no";
                        $resultsl = $this->db->query($sql_l)->getRowArray();
                    
                        if ($resultsl && $resultsl['cnt'] > 0) {
                            $t_lc = $resultsl['cnt'];
                            $t_lcid = $resultsl['lcid'];
                        }
            ...
            if ($check_for_cb == "yes") {
                ...
                $sql2 = "SELECT jcode AS jcode, 
                                CASE 
                                    WHEN (jname LIKE '%CHIEF JUSTICE%' OR jname LIKE '%Registrar%') THEN CONCAT(TRIM(jname), ' (', first_name, ' ', sur_name, ' )') 
                                    ELSE TRIM(jname) 
                                END AS jname 
                        FROM judge 
                        WHERE display = 'Y'  
                            AND jtype IN ('J','R')  
                        ORDER BY IF(is_retired = 'N', 0, 1), jtype, judge_seniority";
            
                $results2 = $this->db->query($sql2)->getResultArray();
            
                $tjud1 = $tjud2 = $tjud3 = $tjud4 = $tjud5 = "";
                $cljudge1 = $cljudge2 = $cljudge3 = $cljudge4 = $cljudge5 = '';
                $djcnt = 0;
            
                foreach ($results2 as $row2) {
                    $jcode = $row2['jcode'];
                    $jname = str_replace("\\", "", $row2['jname']);
            
                    $html_bunch_cases .= '<option value="' . $jcode . '||' . $jname . '">' . $jname . '</option>';
            
                    // Check for selected judges
                    if ($cljudge1 == $jcode) {
                        $tjud1 = '<input type="checkbox" id="hd_chk_jd1" onclick="getDone_upd_cat(this.id);" checked="true" value="' . $jcode . '||' . $jname . '"/>&nbsp;<font color="yellow"><b>' . $jname . '</b></font>';
                        $djcnt++;
                    }
                    if ($cljudge2 == $jcode) {
                        $tjud2 = '<input type="checkbox" id="hd_chk_jd2" onclick="getDone_upd_cat(this.id);" checked="true" value="' . $jcode . '||' . $jname . '"/>&nbsp;<font color="yellow"><b>' . $jname . '</b></font>';
                        $djcnt++;
                    }
                    if ($cljudge3 == $jcode) {
                        $tjud3 = '<input type="checkbox" id="hd_chk_jd3" onclick="getDone_upd_cat(this.id);" checked="true" value="' . $jcode . '||' . $jname . '"/>&nbsp;<font color="yellow"><b>' . $jname . '</b></font>';
                        $djcnt++;
                    }
                    if ($cljudge4 == $jcode) {
                        $tjud4 = '<input type="checkbox" id="hd_chk_jd4" onclick="getDone_upd_cat(this.id);" checked="true" value="' . $jcode . '||' . $jname . '"/>&nbsp;<font color="yellow"><b>' . $jname . '</b></font>';
                        $djcnt++;
                    }
                    if ($cljudge5 == $jcode) {
                        $tjud5 = '<input type="checkbox" id="hd_chk_jd5" onclick="getDone_upd_cat(this.id);" checked="true" value="' . $jcode . '||' . $jname . '"/>&nbsp;<font color="yellow"><b>' . $jname . '</b></font>';
                        $djcnt++;
                    }
                }
                ...
                <tr>
                    <td align="center">
                        <input class="cls_chkd" name="chkd41" id="chkd41" value="41|Leave granted" type="checkbox"><label class="lblclass" for="chkd41">Leave granted</label><br>
                        <input class="cls_chkd" name="chkd178" id="chkd178" value="178|Leave Granted &amp; Allowed" type="checkbox"><label class="lblclass" for="chkd178">Leave Granted &amp; Allowed</label><br>
                        <input class="cls_chkd" name="chkd176" id="chkd176" value="176|Leave Granted &amp; Disposed off" type="checkbox"><label class="lblclass" for="chkd176">Leave Granted &amp; Disposed off</label><br>
                        <input class="cls_chkd" name="chkd177" id="chkd177" value="177|Leave Granted &amp; Dismissed" type="checkbox"><label class="lblclass" for="chkd177">Leave Granted &amp; Dismissed</label>
                    </td>
                </tr>
            </table>';
    ...
    function get_casedesc($data){$dataArr = [];
